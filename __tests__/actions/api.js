...
async function test(state, obj, request, error, method, methodParams, apiResult, query, data) {
  store = mockStore({ auth: { ...mockAuthState, ...state } });
  mockFnWithParams(API_CALLS, request.name, Promise.reject({ apiError: error }), query, data);
...
it('should update token if present in response', async() => {
  ...
  mockFnWithParams(API_CALLS, getMeRequest.name, Promise.resolve({ sessionHeader: newToken }), accessTokenQuery, {});
